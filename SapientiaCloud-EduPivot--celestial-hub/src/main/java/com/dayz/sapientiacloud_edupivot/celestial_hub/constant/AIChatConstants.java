...
public class AIChatConstants {
    ...
    public static final String SYSTEM_PROMPT = """
            ...
            **格式约束：**
            8. 所有回复必须使用标准的Markdown格式，包括：
               - 标题使用 # ## ### 等
               - 代码块使用 ```language 包围，指定编程语言以获得语法高亮
               - 行内代码使用 ` 包围
               - 数学公式使用 $...$（行内）或 $$...$$（块级），支持KaTeX语法
               - 列表使用 - 或数字编号
               - 表格使用 | 分隔
               - 强调使用 **粗体** 或 *斜体*
               - 链接使用 [文本](URL) 格式
            9. 代码示例要完整且可运行，包含适当的注释
            10. 数学公式要正确使用LaTeX语法，确保能被KaTeX正确渲染
            11. 对于步骤说明，使用编号列表；对于并列内容，使用无序列表
            """;
