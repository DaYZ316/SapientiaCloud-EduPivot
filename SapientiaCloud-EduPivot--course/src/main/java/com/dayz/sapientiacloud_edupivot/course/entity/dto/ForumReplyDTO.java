...
@Data
@Schema(description = "论坛回复数据传输对象")
public class ForumReplyDTO implements Serializable {

    @Serial
    private static final long serialVersionUID = -1862468525476173967L;

    @Schema(name = "id", description = "回复ID，更新时必须提供")
    private UUID id;
    ...
    @Min(value = 0, message = "采纳标识输入不正确")
    @Max(value = 1, message = "采纳标识输入不正确")
    private Integer isAccepted;

    @Schema(name = "status", description = "回复状态 (0=正常, 1=删除, 2=审核中, 3=审核失败)")
    @Min(value = 0, message = "回复状态输入不正确")
    @Max(value = 3, message = "回复状态输入不正确")
